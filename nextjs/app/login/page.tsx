...
const YANDEX_REDIRECT_URI = process.env.NEXT_PUBLIC_YANDEX_REDIRECT_URI!;
...
export default function Login() {
    ...
    useEffect(() => {
        ...
        if (token) {
            ...
            const redirectPath = redirectParam && redirectParam !== "/login" ? redirectParam : "/projects";
            router.push(redirectPath);
        }
    }, [router, searchParams]);

    const handleYandexLogin = () => {
        // Сохраняем redirect параметр в sessionStorage для использования после авторизации
        const redirectParam = searchParams.get("redirect");
        if (redirectParam) {
