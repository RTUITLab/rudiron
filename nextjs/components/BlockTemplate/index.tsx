...
export default function BlockTemplate({
    ...
    const childrenByFieldKey = useRef<string>("");

    useEffect(() => {
        const currentKey = JSON.stringify(
            Object.keys(childrenByField)
                .sort()
                .map(key => [key, childrenByField[key].map(c => ({id: c.id, code: c.code}))])
        );

        if (currentKey === childrenByFieldKey.current) {
            return;
        }
        childrenByFieldKey.current = currentKey;
                    ...
                    <button onClick={() => deleteBlock(id)} title="Удалить блок">
                        <svg width="28" height="28" viewBox="0 0 28 28" fill="none" xmlns="http://www.w3.org/2000/svg">
                            ...
                            <path
                                ...
                                stroke="white" strokeWidth="1.5" strokeLinecap="round"/>
                            <path d="M11.082 19.25L11.082 12.25" stroke="white" strokeWidth="1.5"
                                  strokeLinecap="round"/>
                            <path d="M16.918 19.25L16.918 12.25" stroke="white" strokeWidth="1.5"
                                  strokeLinecap="round"/>
                        </svg>
                    </button>
                </div>
                            ...
                            <BlockAttachments
                                ...
                                onBlocksChange={(blocks) => {
                                    setNestedBlocksByField((prev) => {
                                        ...
                                        // Если блоки для этого поля не изменились, не обновляем состояние
                                        if (prevKey === newKey) {
                                            return prev;
                                        }
