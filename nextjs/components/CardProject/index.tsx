...
export default function CardProject({id, name, description, liked, onEdit, onDelete, onLikeChange}: Props) {
    ...
    const handleCardClick = () => {
        if (!menuOpen && id && typeof id === 'string') {
            router.push(`/project/${id}`);
        }
    };

    const likeProject = async (e: React.MouseEvent) => {
        e.stopPropagation();
    ...
    return (
        <div className={Style.card} onClick={handleCardClick}>
            ...
            >
                ...
                <div style={{position: "absolute", top: 16, left: 16, zIndex: 9999, width: 32}} onClick={likeProject}>
                    {!isLiked ? (
